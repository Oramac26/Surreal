...
int MenuScr::Run(sf::RenderWindow &win){
    ...
    while( win.isOpen() )
    {
		...
		if (options==true) {
			...
			if (CorrectM(on, win)&&( zdarzenie.type == sf::Event::MouseButtonReleased && zdarzenie.mouseButton.button == sf::Mouse::Left )) mus=true;
			if (CorrectM(off, win)&&( zdarzenie.type == sf::Event::MouseButtonReleased && zdarzenie.mouseButton.button == sf::Mouse::Left )) mus=false;		
			...
			if (CorrectM(exit, win)) {
				win.draw( exitcl );
				if ( zdarzenie.type == sf::Event::MouseButtonPressed && zdarzenie.mouseButton.button == sf::Mouse::Left ) win.close();
			}
		}
		win.display();
    }
